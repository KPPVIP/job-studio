...
local open = false 
local mainMenu = RageUI.CreateMenu('~b~Accueil', '~b~studio') 
local subMenu1 = RageUI.CreateMenu('~b~Rendez Vous', '~b~studio') 
...
function OpenMenuAccueilstudio() 
    if open then 
		open = false
        ...
        RageUI.Button("Appeler un employ� a l'accueil", nil, {RightLabel = "→→"}, not codesCooldown5 , {
            onSelected = function()
            codesCooldown5 = true 
        TriggerServerEvent('Appel:studio')
        ...
        RageUI.IsVisible(subMenu1, function()    
            ...
            RageUI.Button("Valider la Demande", "~o~Merci de respecter les ambulancier.", { Color = {BackgroundColor = { 76, 175, 80, 50}} }, true, {
                onSelected = function()
                    ...
                    RageUI.CloseAll()
                    TriggerServerEvent("Rdv:studioMotif", nomprenom, numero, heurerdv, rdvmotif)
                    ESX.ShowNotification("~b~studio~s~\nDemande de Rendez-vous envoyée")
                    nomprenom = nil
                    numero = nil
                    heurerdv = nil
                    rdvmotif = nil
